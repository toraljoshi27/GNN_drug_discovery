...
def smiles_to_ecfp(smiles_list: List[str], n_bits: int = 2048, radius: int = 2) -> np.ndarray:
    """
    Convert a list of SMILES into ECFP fingerprints (Morgan) using the new MorganGenerator API.
    """
    ...
    for smi in smiles_list:
        ...
        if mol is None:
            ...
            continue
        # Get fingerprint as numpy array directly
...
def run_esol_baselines(seeds: List[int]) -> Dict[str, Dict[str, Dict[str, float]]]:
    ...
    for seed in seeds:
        ...
        mlp = MLPRegressor(
            ...
            activation="relu",
            max_iter=500,
            learning_rate_init=1e-4,  # Lower LR for stability
        ...
        def train_and_predict_mlp():
            ...
            for t in range(T):
                ...
                clf = MLPClassifier(
                    ...
                    activation="relu",
                    max_iter=500,
                    learning_rate_init=1e-4,  # Lower LR for better calibration
                    early_stopping=True,       # Prevent overfitting
                    validation_fraction=0.1,
                    alpha=1e-4,                # L2 regularization
                    random_state=seed,
...
def main():
    ...
    # Try to load existing combined GNN summary (check multiple possible paths)
    results_dir = Path("results")
    possible_paths = [
        results_dir / "combined_all_datasets_with_tox21.xlsx",
        results_dir / "combined" / "all_datasets_summary.xlsx",
    ]
    
    df_gnn = None
    for path in possible_paths:
        if path.exists():
            print(f"Found existing GNN summary at {path}")
            # Try to read the summary sheet
            try:
                df_gnn = pd.read_excel(path, sheet_name="summary_all")
            except Exception:
                # Try reading first sheet if summary_all doesn't exist
                df_gnn = pd.read_excel(path, sheet_name=0)
            break
    ...
    if df_gnn is not None:
        ...
        out_path = results_dir / "combined_all_with_baselines.xlsx"
        with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
            df_all_with_baselines.to_excel(writer, sheet_name="summary_all", index=False)
            df_gnn.to_excel(writer, sheet_name="gnn_only", index=False)
        ...
        # No existing GNN summary found - just save baselines
        print("[INFO] No existing GNN summary found. Saving baselines only.")
        out_path = results_dir / "baselines_summary.xlsx"
        with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
            df_baselines.to_excel(writer, sheet_name="baselines", index=False)
        
        print(f"[OK] Wrote baseline summary to {out_path}")
